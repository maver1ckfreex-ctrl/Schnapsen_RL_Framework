...
from schnapsen.bots import RdeepBot,RandBot, BullyBot
...
MATRIX_FILE = "winning_rate_matrix_10k_Base.csv"
...
def play_matchup_chunk(args):
    ...
    def load_bot_from_config(cfg, seed_offset):
        ...
        if cfg['type'] == 'random':
            return RandBot(rng, name=cfg['name'])
        elif cfg['type'] == 'bully':
            return BullyBot(rng, name=cfg['name'])
        elif cfg['type'] == 'rdeep':
...
# --- A. DEFINE THE ROSTER ---
    ...
    for p1_conf in bot_registry:
        for p2_conf in bot_registry:
    ...
    with open(MATRIX_FILE, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Player_vs_Against"] + player_names)
