...
MATRIX_FILE = "winning_rate_matrix_10k_SL.csv"
...
class StandardMLP(nn.Module):
    def __init__(self):
        super().__init__()
          # 173 -> 1 (30 neurons) training based on 173d feature vector || 1 (win) or 0 (lose)
        self.classifier = nn.Sequential(
            nn.Linear(INPUT_DIM, 512),         # MLP hidden layer
            nn.ReLU(),                        # MLP non-linear activation function
            nn.Linear(512, 1)                  # MLP output layer 
        )
...
def play_matchup_chunk(args):
    ...
    def load_bot_from_config(cfg, seed_offset):
        ...
        if cfg['type'] == 'rdeep':
            return RdeepBot(num_samples=4, depth=cfg['depth'], rand=rng, name=cfg['name'])
        elif cfg['type'] == 'mlp':
...
# --- A. DEFINE THE ROSTER ---
    bot_registry_1 = [ 
        #(Standard MLP)
        {'name': 'MLP-D2',          'type': 'mlp',    'path': MODEL_DIR / 'MLP_dp2.pth'},
        {'name': 'MLP-D4',          'type': 'mlp',    'path': MODEL_DIR / 'MLP_dp4.pth'},
        {'name': 'MLP-D6',          'type': 'mlp',    'path': MODEL_DIR / 'MLP_dp6.pth'},
        
    ]
    bot_registry_2 = [
        {'name': 'Rdeep-D2',        'type': 'rdeep', 'depth': 2},
        {'name': 'Rdeep-D4',        'type': 'rdeep', 'depth': 4},
        {'name': 'Rdeep-D6',        'type': 'rdeep', 'depth': 6},
    ...
    print(f"Opponents: {len(opponent_names)}")
